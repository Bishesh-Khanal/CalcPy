...
data_types.forEach((data_type) => {
  data_type.addEventListener("change", function () {
    ...
    let checked_radio = document.querySelector(
...
compute_btn.addEventListener("click", function () {
  ...
  if (checked_radio == "discreete") {
    ...
    for (let i = 0; i < data.length; i++) {
      if (
        isNaN(data[i][0]) ||
        isNaN(data[i][1] || data[i][0] == "" || data[i][1] == "")
      ) {
    ...
    if (document.getElementById("bar-graph-render") != null) {
      document.getElementById("bar-graph-render").remove();
    }
    if (document.getElementById("box-plot-render") != null) {
      document.getElementById("box-plot-render").remove();
    }

    // create the canvases
    document.getElementById("bar-graph").innerHTML +=
      '<canvas id = "bar-graph-render" height = "400" width = "400"></canvas>';
    document.getElementById("box-plot").innerHTML +=
      '<canvas id = "box-plot-render" height = "400" width = "400""></canvas>';
  ...
  if (checked_radio == "class-interval") {
    ...
    for (let i = 0; i < cont_data.length; i++) {
      mean += cont_data[i][4] * cont_data[i][2];
    }
    ...
    for (let i = 0; i < cont_data.length; i++) {
      if (cont_data[i][3] == q1_closest_cf) {
        ...
        if (i != 0) {
          q1_previous_cf = cont_data[i - 1][3];
        }
        break;
      }
    }
    ...
    let q1 =
      q1_lower_limit +
      ((q1_index - q1_previous_cf) / q1_frequency) *
        (q1_upper_limit - q1_lower_limit);
    ...
    for (let i = 0; i < cont_data.length; i++) {
      if (cont_data[i][3] == median_closest_cf) {
        ...
        if (i != 0) {
          median_previous_cf = cont_data[i - 1][3];
        }
        break;
      }
    }
    ...
    let median =
      median_lower_limit +
      ((median_index - median_previous_cf) / median_frequency) *
    ...
    for (let i = 0; i < cont_data.length; i++) {
      if (cont_data[i][3] == q3_closest_cf) {
        ...
        if (i != 0) {
          q3_previous_cf = cont_data[i - 1][3];
        }
        break;
      }
    }
    ...
    let q3 =
      q3_lower_limit +
      ((q3_index - q3_previous_cf) / q3_frequency) *
        (q3_upper_limit - q3_lower_limit);

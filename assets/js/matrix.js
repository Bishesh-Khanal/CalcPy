...
function generate_matrix(row, col, id) {
  ...
  html +=
    '<div id = "' +
    id +
    '-generated-matrix"class = " mb-4 grid grid-rows-' +
    row +
    ' gap-3 mt-2"> \n \n';
  for (let i = 0; i < row; i++) {
    ...
    for (let j = 0; j < col; j++) {
      html +=
        ...
        (i + 1) +
        (j + 1) +
        '" class="block mt-2 w-12 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"> \n';
    }
    html += "</div> \n \n";
  }
  html += "</div>\n";
...
function array_to_matrix(arr, matrix_name_id) {
  ...
  for (let i = 0; i < row; i++) {
    for (let j = 0; j < col; j++) {
      document
        .getElementById(matrix_name_id + "-generated-matrix")
        .querySelector("#inp-" + (i + 1) + (j + 1)).value = arr[i][j];
    }
  }
...
first_generation_button.addEventListener("click", function () {
  ...
  // if row/col > 6, then show a toast
  if (row > 6 || col > 6) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator(
        "You may generate matrices with maximum 6 rows and 6 columns!"
      )
    );
  ...
  if (!number_validator(row) || !number_validator(col)) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please enter valid inputs!")
    );
...
function inverse(matrix) {
  ...
  const identity = new Array(n)
    .fill()
    .map((_, i) => new Array(n).fill().map((_, j) => (i === j ? 1 : 0)));
...
solve_button.addEventListener("click", function () {
  ...
  if (
    document.getElementById("matrix-1-generated-matrix") == null ||
    document.getElementById("matrix-2-generated-matrix") == null
  ) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please generate the matrices first!")
    );
  ...
  if (!matrix_validator(mat1) || !matrix_validator(mat2)) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please enter valid matrix values!")
    );
  ...
  if (operator == "plus") {
    if (row_col_equal_checker(mat1, mat2)) result = add_matrices(mat1, mat2);
      ...
      document.body.insertAdjacentHTML(
        "beforeend",
        toast_generator("The matrices must have the same dimensions!")
      );
    ...
    if (row_col_equal_checker(mat1, mat2))
...
mat1_det_btn.addEventListener("click", function () {
  ...
  if (document.getElementById("matrix-1-generated-matrix") == null) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please generate the matrix first!")
    );
...
ans_det_btn.addEventListener("click", function () {
  ...
  if (document.getElementById("answer-generated-matrix") == null) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please compute first!")
    );
...
mat1_inv_btn.addEventListener("click", function () {
  ...
  if (!square_matrix_checker(mat1)) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("Please enter a square matrix!")
    );
  ...
  if (det == 0) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("The matrix is singular!")
    );
...
mat2_inv_btn.addEventListener("click", function () {
  ...
  if (!square_matrix_checker(mat2)) {
    document.body.insertAdjacentHTML(
      "beforeend",
      toast_generator("The matrix is not square!")
    );
